...
int disastrOS_syscall(int syscall_num, ...) {
  ...
  if (syscall_num<0||syscall_num>DSOS_MAX_SYSCALLS)
	...
	/*avendo ogni syscall un proprio numero di argomenti (che di norma vede dalla CPU), allora 		questa funzione dovra' avere un numero variabile di argomenti (il syscall num, + tutti gli 		argomnti di quella specifica syscall*/
...
void disastrOS_start(void (*f)(void*), void* f_args, char* logfile){  
  ...
  disastrOS_debug("initializing system structures\n");
	//Se scrivi qualcosa nella funzione disastrOS debug, allora il risultato lo si puo' trovare
	//nel 3o argomento della funzione char* logfile
  PCB_init();
  Timer_init();
  Resource_init();
  Descriptor_init();
  ...
  for (int i=0; i<DSOS_MAX_SYSCALLS; ++i){
	...
	//Ogni volta che faccio disastrOS_start azzero il vettore delle syscall (con il ciclo for qui 
	//sopra) e lo "ripopolo" con le syscall che indico qui sotto
  syscall_vector[DSOS_CALL_PREEMPT]   = internal_preempt;
  syscall_numarg[DSOS_CALL_PREEMPT]   = 0;		//per ogni syscall indico sia il suo nome che 								//il numero di argomenti che prende
